...
/** @addtogroup AD5941_System_Examples
* @{
...
*  ## Sequence Allocation
* SEQ3 is used to initialize AD5941.\n
...
* @details Do not modify following default parameters. Use the function in AD5941Main.c to change it. 
* 
* */
...
* @return none.
*/
AD5941Err AppSWVGetCfg(void *pCfg)
{
  if(pCfg)
  {
...
* @return none.
*/
AD5941Err AppSWVCtrl(uint32_t Command, void *pPara)
{
  switch (Command)
  {
  case APPCTRL_START:
...
*/
static AD5941Err AppSWVSeqInitGen(void)
{
  ...
  {
    AD5941_StructInit(&AppSWVCfg.InitSeqInfo, sizeof(AppSWVCfg.InitSeqInfo));
    ...
    AD5941_SEQInfoCfg(&AppSWVCfg.InitSeqInfo);
  }
  else
    return error; /* Error */
...
* @return return error code.
*/
static AD5941Err AppSWVSeqADCCtrlGen(void)
{
  AD5941Err error = AD5941ERR_OK;
  const uint32_t *pSeqCmd;
  uint32_t SeqLen;
  ...
  AD5941_SEQGenInsert(SEQ_WAIT(16*100));  /* wait 250us for reference power up */
  ...
  if(error == AD5941ERR_OK)
  {
    AD5941_StructInit(&AppSWVCfg.ADCSeqInfo, sizeof(AppSWVCfg.ADCSeqInfo));
    ...
    AD5941_SEQInfoCfg(&AppSWVCfg.ADCSeqInfo);
  }
  else
    return error; /* Error */
...
* @return return error code.
*/
static AD5941Err RampDacRegUpdate(uint32_t *pDACData)
{
  ...
  //AppSWVCfg.CurrRampVal = AppSWVCfg.CurrRampVal + AppSWVCfg.DACCodePerRamp;																							|
	//when bSqrWaveHiLevel is true, the DACOutVal is the current staircase (CurrRampVal) plus amplitude (DACCodePerStep) ---|
  if(AppSWVCfg.bSqrWaveHiLevel)
  {   
		AppSWVCfg.DACOutputVal = AppSWVCfg.CurrRampVal + AppSWVCfg.DACCodePerStep;
		
       AppSWVCfg.bSqrWaveHiLevel = bFALSE;
  }else // when bSqrWaveHiLevel is false, the DACOutVal is current staircase (CurrRampVal) minus amplitude,	___ then stair case (CurrRampVal) increases a step (DACCodePerRamp)
		//																																																			   |
	 //																																																					 |
  {
  ...
  VbiasCode = (uint32_t)(VzeroCode*64 + AppSWVCfg.DACOutputVal);  // VbiasCode 
...
* 
* */
static AD5941Err AppSWVSeqDACCtrlGen(void)
{  
...
#define CURRBLK_BLK1      1   /* Current block is BLOCK1 */
  ...
  if(AppSWVCfg.bFirstDACSeq == bTRUE)
  {
   ...
   // AppSWVCfg.CurrRampCode = AppSWVCfg.RampStartVolt/DAC12BITVOLT_1LSB;
		AppSWVCfg.CurrRampVal = AppSWVCfg.RampStartVolt/DAC12BITVOLT_1LSB;
...
if(AppSWVCfg.bFirstDACSeq)
{
  ...
  AD5941_SEQInfoCfg(&AppSWVCfg.DACSeqInfo);
}
...
* @return return error code.
*/
static AD5941Err AppSWVRtiaCal(void)
{
...
* @return return error code.
*/
AD5941Err AppSWVInit(uint32_t *pBuffer, uint32_t BufferSize)
{
  AD5941Err error = AD5941ERR_OK;  
  ...
  AD5941_SEQMmrTrig(AppSWVCfg.InitSeqInfo.SeqId);
...
static int32_t AppSWVDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  {
    pData[i] &= 0xffff;
    pOut[i] = AD5941_ADCCode2Volt(pData[i], AppSWVCfg.AdcPgaGain, AppSWVCfg.ADCRefVolt)/AppSWVCfg.RtiaValue.Magnitude * 1e3f;  /* Result unit is uA. */
  }
  return 0;
}
...
uint16_t endseq = 0;
uint32_t TotalCnt = 0;
AD5941Err AppSWVISR(void *pBuff, uint32_t *pCount)
{
  uint32_t BuffCount;
  uint32_t FifoCnt;
  BuffCount = *pCount;
  ...
  *pCount = 0;
  ...
  if(IntFlag&AFEINTSRC_DATAFIFOTHRESH)
  {
    ...
    if(FifoCnt > BuffCount)
    {
      ///@todo buffer is limited.
		...
		uint32_t *pAdjustedBuff_t = (uint32_t *)pBuff + TotalCnt;
		AD5941_FIFORd(pAdjustedBuff_t, FifoCnt);
  ...
  if(IntFlag & AFEINTSRC_ENDSEQ)
  {
		...
		FifoCnt = AD5941_FIFOGetCnt();
    AD5941_INTCClrFlag(AFEINTSRC_ENDSEQ);
		uint32_t *pAdjustedBuff = (uint32_t *)pBuff + TotalCnt;
		
		AD5941_FIFORd(pAdjustedBuff, FifoCnt);
		TotalCnt += FifoCnt;
   	*pCount = TotalCnt;

		endseq = 1;
    
		AppSWVCtrl(APPCTRL_SHUTDOWN, 0); /*stop AD5941 */
